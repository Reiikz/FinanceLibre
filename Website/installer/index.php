...
if(isset($_POST["install"])){
    ...
    query($conn, "CREATE TABLE IF NOT EXISTS `FinanceLibre`.`" . $db_prefix . "account` ( `id` INT NOT NULL AUTO_INCREMENT, `user_id` INT NOT NULL, `name` VARCHAR(100) NOT NULL, `currency` VARCHAR(45) NULL, `balance` DOUBLE NULL, `type` VARCHAR(45) NOT NULL, PRIMARY KEY (`id`), INDEX `fk_account_user_idx` (`user_id` ASC) VISIBLE, CONSTRAINT `fk_account_user` FOREIGN KEY (`user_id`) REFERENCES `FinanceLibre`.`fcl_user` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE = InnoDB;");
